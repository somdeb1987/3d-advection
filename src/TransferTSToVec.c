...
int TransferTSToVec(DM da,Vec u,Vec v,Vec w,Vec phi,Vec Y,void *ctxt) 
{
...
*/
  ...
  for (i=istart; i<iend; i++) {
    for (j=jstart; j<jend; j++) {
		...
		arru[k][j][i] = Yarr[offset_u+y_index];
        	y_index++;
	}
    }
  }
  ...
  for (i=istart; i<iend; i++) {
    for (j=jstart; j<jend; j++) {
    	for (k=kstart; k<kend; k++) {
        	arrv[k][j][i] = Yarr[offset_v+y_index];
        	y_index++;
	}
    }
  }
  ...
  for (i=istart; i<iend; i++) {
    for (j=jstart; j<jend; j++) {
    	for (k=kstart; k<kend; k++) {
        	arrw[k][j][i] = Yarr[offset_w+y_index];
        	y_index++;
	}
    }
  }
  ...
  for (i=istart; i<iend; i++) {
    for (j=jstart; j<jend; j++) {
    	for (k=kstart; k<kend; k++) {
        	arrphi[k][j][i] = Yarr[offset_phi+y_index] ;
        	y_index++;
	}
    }
  }
